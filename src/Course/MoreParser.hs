...
between ::
  ...
  -> Parser a
  -> Parser a
between po pc pa = po >> (pa >>= \a -> pc >> pure a)
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 pa ps = (pa >>= pure) .:. list (ps >> pa)
...
-- satisfyAll preds = P (\case 
--     Nil -> UnexpectedEof
--     (c:.cs) -> (\f -> if and (f c) then Result cs c else UnexpectedChar c) (sequence preds)
--   )
-- satisfyAll preds = P (\case
--     Nil -> UnexpectedEof
--     (c:.cs) -> if all (\pred -> pred c) preds then Result cs c else UnexpectedChar c
--   )
