...
(P p1) ||| (P p2) = P (\s ->
  let p1Result = p1 s
  in if isErrorResult (p1 s) then p2 s else p1 s)
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  f =<< (P p) = P (\s -> onResult (p s) (\s2 a -> parse (f a) s2))
...
-- firstNameParser = upper >>= (\s -> pure s .:. list lower)
...
--   _ <- spaces1 
--   phone <- phoneParser
--   pure $ Person age firstName surname isSmoker phone
personParser = 
  ageParser >>=~ (\age -> 
