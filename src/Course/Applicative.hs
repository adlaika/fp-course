...
--
filtering :: forall k a.
  Applicative k =>
  (a -> k Bool)
  -> List a
  -> k (List a)
filtering kpred = foldRight 
  (\nxt -> lift2 (\b -> if b then (:.) nxt else id) (kpred nxt)) 
  (pure Nil)

-- fold over each item in the List a
-- starting with a k (List a), because you have to have a context to transform into:
-- call the k predicate, then decide whether or not to cons based on the Bool inside that result by using lift2 to access it
