...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  f <$> st = StateT (\s -> first f <$> runStateT st s)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  StateT stF <*> StateT stA = StateT (\s -> do 
      (f, s') <- stF s 
      (a, s'') <- stA s'
      pure (f a, s'')
    )
...
instance Functor k => Functor (OptionalT k) where
  (<$>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  f <$> o = (\k -> OptionalT $ (f <$>) <$> k) $ runOptionalT o 
...
instance Monad k => Applicative (OptionalT k) where
  ...
  OptionalT f <*> OptionalT a = OptionalT $ optional (\f'' -> (f'' <$>) <$> a) (pure Empty) =<< f
  -- (OptionalT f) <*> (OptionalT o) = OptionalT $ do
  --   o' <- o
  --   f' <- f
  --   pure (f' <*> o')
  -- attempt at using onFull:
  -- (OptionalT f) <*> (OptionalT o) = OptionalT $ o >>= (\x -> f >>= (\g -> onFull (\a -> pure (g >>= (\ab -> pure $ ab a))) x)) 
...
instance Monad k => Monad (OptionalT k) where
  (=<<) :: forall a b.
    ...
    -> OptionalT k a
    -> OptionalT k b
  -- below causes an infinite loop for reasons I don't fully understand
  -- f =<< (OptionalT k) = join $ OptionalT $ (f <$>) <$> k
  f =<< (OptionalT k) = OptionalT $ k >>= \o -> case o of 
...
instance Applicative (Logger l) where
  ...
  (<*>) ::
    ...
    -> Logger l a
    -> Logger l b
  Logger l f <*> Logger l2 a = Logger (l ++ l2) (f a)
...
distinctG as = runOptionalT (evalT (filtering (
  \a -> StateT (
    \s -> OptionalT (
      if a>100 
      then Logger (("aborting > 100: " ++ toChars a):.Nil) Empty 
      else (if even a
        then Logger (("even number: " ++ toChars a):.Nil)
        else pure) (Full (S.notMember a s, S.insert a s))
    )
  )) as) S.empty)
  where 
    toChars :: a -> Chars
    toChars x = listh $ show x
